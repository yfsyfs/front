<template>
  <div class="shop-header">
    <!--虽然 info是异步加载的, 所以这里 info.bgImg 初始为undefined, 但是undefined并不会报错, undefined就undefined嘛，大不了不显示而已-->
    ...
    <!--因为 info是异步加载的（在Shop组件中异步加载），所以这里info.supports是undefined,
    所以我们要等到 info.supports有数据了 再显示,不然报错, 所以这里有v-if, 虽然报错，报的是初始化显示的错误, 后面数据上来了,
    依旧是可以正常显示的, 但是为了避免这种情况，我们还是v-if处理一下. 注意, 不能使用v-show, 因为v-show 仍然会解析, 只是不显示而已, 只要解析, 就会报错 -->
      ...
      <div class="activity-sheet" v-show="supportShow">
        <div class="activity-sheet-content">
          ...
          <div class="activity-sheet-close">
					<span class="iconfont icon-close" @click="supportShow=!supportShow">
					</span>
          </div>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        shopShow: false,  // 初始店铺介绍是不展示的
        supportShow: false // 初始优惠活动都是不展示的
      }
    }
  }
</script>
